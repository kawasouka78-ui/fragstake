...
export function createWeaponFinish(skin?: string) {
  ...
  const wrap =
    wrapId && Object.hasOwn(weaponWraps, wrapId)
      ? weaponWraps[wrapId]
      : undefined;
  const motionPattern =
    effect && Object.hasOwn(patterns, effect) ? patterns[effect] : undefined;
  const pattern = motionPattern ?? wrap?.fragment;
  const animated = motionPattern !== undefined;
  const material = new THREE.MeshStandardMaterial({
    color: pattern ? '#ffffff' : skin?.startsWith('#') ? skin : '#050607',
    metalness: wrap?.metalness ?? (animated ? 0.28 : 0.58),
    roughness: wrap?.roughness ?? (animated ? 0.42 : 0.38),
  });
  const time = { value: 0 };
  if (pattern) {
    ...
    material.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader =
        `${animated ? 'uniform float uFinishTime;' : ''}
    varying vec3 vFinishPosition;
