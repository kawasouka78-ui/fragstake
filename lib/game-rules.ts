...
export type Result = { live?:boolean;liveGuest?:boolean;kills:number; deaths:number; balance:number; reason:string; won:boolean; score:number; enemyScore:number;headshots?:number;maxStreak?:number;elapsed?:number;net?:number;returned?:number;walletBalance?:number;mode?:Mode;mapId?:string;rate?:number;entry?:number;lastDeathLoss?:number };
