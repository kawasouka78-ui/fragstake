...
export type MatchConfig = { mode: Mode; rate: number; team: '1v1' | '2v2'; balance: number; mapId?:string;stake?:number;target?:number;bestOf?:number;weaponRule?:string;entry?:number;skin?:string;knifeStyle?:'standard'|'karambit';live?:{ticket:string;url:string;mode:'practice'|'ffa'|'1v1'|'2v2';guest:boolean} };
export type Result = { rematch?:NonNullable<MatchConfig['live']>&{expiresAt:number};liveMode?:'practice'|'ffa'|'1v1'|'2v2';placement?:number;standings?:{id:number;name:string;team:number;you:boolean;kills:number;deaths:number}[];live?:boolean;liveGuest?:boolean;kills:number; deaths:number; balance:number; reason:string; won:boolean; score:number; enemyScore:number;headshots?:number;maxStreak?:number;elapsed?:number;net?:number;returned?:number;walletBalance?:number;mode?:Mode;mapId?:string;rate?:number;entry?:number;lastDeathLoss?:number };
