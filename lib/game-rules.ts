...
export type MatchConfig = { mode: Mode; rate: number; team: '1v1' | '2v2'; balance: number; mapId?:string;stake?:number;target?:number;bestOf?:number;weaponRule?:string;entry?:number;skin?:string };
export type Result = { kills:number; deaths:number; balance:number; reason:string; won:boolean; score:number; enemyScore:number;headshots?:number;maxStreak?:number;elapsed?:number;net?:number;returned?:number;walletBalance?:number;mode?:Mode;mapId?:string;rate?:number;entry?:number;lastDeathLoss?:number };
