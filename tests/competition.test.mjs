...
test('eight weapons have ammunition and pistol rules prevent rifle selection',()=>{assert.equal(weaponIds.length,8);const g=new Simulation({...config,weaponRule:'pistol'});for(const id of weaponIds){assert.ok(weapons[id].damage>0);assert.ok(g.ammo[id]>0)}assert.deepEqual(g.allowedWeapons,['pistol','handcannon']);g.switchWeapon('marksman');assert.equal(g.weapon,'pistol');g.switchWeapon('handcannon');assert.equal(g.weapon,'handcannon')});
