...
for(const style of ['standard','karambit'])test(style+' knife has finite geometry and a resetting slash with no magazine motion',()=>{
 ...
 assert.ok(draws>0&&draws<20);assert.ok(model.rig.muzzle.z<-.2);assert.equal(model.userData.knifeStyle,style);
 ...
 const box=new THREE.Box3().setFromObject(bare),size=box.getSize(new THREE.Vector3());assert.ok(size.z>.4&&size.z<.6);assert.ok(size.x<.06);
