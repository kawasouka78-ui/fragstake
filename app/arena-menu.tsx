...
export default function ArenaMenu({game,config,map,ready,error,settings,prefs,setSettings,preference,select,resume,leave,cashOut}:Props){
  ...
  const [deployIn,setDeployIn]=useState(10),resumeRef=useRef(resume);
  resumeRef.current=resume;
  useEffect(()=>{if(game?.started||!isDuel)setDeployIn(10);},[game?.started,isDuel]);
  useEffect(()=>{
    if(!ready||!game||game.started||!isDuel||settings||error)return;
    setDeployIn(10);
    const timer=window.setInterval(()=>setDeployIn(n=>{if(n<=1){window.clearInterval(timer);resumeRef.current();return 0;}return n-1;}),1000);
    return()=>window.clearInterval(timer);
  },[ready,game,isDuel,settings,error]);
  return <div className="fps-menu-scrim deployment-scrim"><section className={"deployment-menu"+(game?.started?" is-paused":"")} aria-label="Arena menu">
    <header className="deployment-heading"><div><span className="deployment-eyebrow">{game?.started?(config.live?'LIVE MATCH · MENU OPEN':'MATCH PAUSED'):'MATCH SETUP'}<i/>{mode}</span><h1>{error?'LET’S TRY AGAIN':settings?'GAME SETTINGS':game?.started?(config.live?'YOUR LOADOUT':'MATCH PAUSED'):'CHOOSE YOUR LOADOUT'}<span>.</span></h1></div><div className="deployment-location"><Crosshair size={18}/><div><b>{map.name}</b><span>{config.live?'Free live match · Arena keeps running':round+' · Bot match'}</span></div></div></header>
    {error?<div className="deployment-error" role="alert"><p>{error}</p><button className="secondary" onClick={leave}><ArrowLeft size={17}/> Back to Play</button></div>:<>
      <div className="deployment-body">
        ...
        <section className="loadout-selection" aria-label={settings?'Game settings':'Choose a weapon'}>
          <div className="loadout-section-heading"><h2>{settings?'GAME SETTINGS':'YOUR ARSENAL'}</h2><span>{settings?'Saved on this device':game?.started&&!game.paused?'Gun + knife':`${game?.allowedWeapons.length??'—'} weapons available`}</span></div>
