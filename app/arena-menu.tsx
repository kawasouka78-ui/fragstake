...
export default function ArenaMenu({game,config,map,ready,error,settings,prefs,confirmLeave,setSettings,preference,select,resume,leave,setConfirmLeave}:Props){
  const selected=game?.weapon??(config.weaponRule==='sniper'?'marksman':config.weaponRule==='pistol'?'pistol':'rifle');
  ...
  const helpKeys=game?.allowedWeapons.length??8;
  return <div className="fps-menu-scrim deployment-scrim"><section className="deployment-menu" aria-label="Arena menu">
