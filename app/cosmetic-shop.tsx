...
export default function CosmeticShop({inventory,owned,equipped,busy,signedIn,act}:Props){
 ...
 const items=catalog.filter(item=>(filter==='all'||(filter==='animated')===('effect' in item)));
 const action=(item:Cosmetic)=><button className="secondary compact" disabled={busy||!signedIn} onClick={()=>void act({action:'inventory_equip',sku:equipped===item.sku?'':item.sku},equipped===item.sku?'Black standard finish equipped.':item.name+' equipped for your next match.')}>{equipped===item.sku?'Unequip':'Equip finish'}</button>;
 return <>
  ...
  <div className="cosmetic-grid">{items.map(item=><article className={'account-panel cosmetic-card'+('effect' in item?' animated-cosmetic':'')} key={item.sku} style={{'--wrap':item.color} as React.CSSProperties}>
   ...
   <div className="cosmetic-footer"><strong>{equipped===item.sku?'EQUIPPED':'UNLOCKED'}</strong>{action(item)}</div>
  </article>)}</div>
