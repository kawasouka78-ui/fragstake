...
export default function CosmeticShop({inventory,owned,equipped,busy,signedIn,act}:Props){
 ...
 function action(item:Cosmetic){
  ...
  if(!unlocked(item))return <button className="secondary compact" disabled>Not available yet</button>;
 ...
 return <>
  ...
  <p className="armory-caption">Gun finishes are free for signed-in players. New knife purchases are not available yet.</p>
  <div className="cosmetic-grid">{items.map((item,index)=><article className={'account-panel cosmetic-card'+('effect' in item?' animated-cosmetic':'')+(isKnife(item)?' knife-cosmetic':'')} key={item.sku} style={{'--wrap':item.color,'--item-index':index} as React.CSSProperties}>
   ...
   <div className="cosmetic-footer"><div><strong>{equipped.has(item.sku)?'EQUIPPED':unlocked(item)?'UNLOCKED':'NOT RELEASED'}</strong>{!unlocked(item)&&<small>PURCHASES CLOSED</small>}</div>{action(item)}</div>
  </article>)}</div>
  ...
  <Dialog open={!!inspected} onOpenChange={open=>{if(!open)setInspected(null)}}><DialogContent className="sc-dialog finish-inspector">{inspected&&<>
   ...
   <div className="inspect-footer"><div><strong>{unlocked(inspected)?'UNLOCKED':'NOT RELEASED'}</strong><small>{unlocked(inspected)?isKnife(inspected)?'KNIFE SLOT · PRESS 2 IN MATCH':'GUN FINISH':'PURCHASES CLOSED'}</small></div>{action(inspected)}</div>
