...
import {ffaCashoutPreview,matchDuration,signedEuros} from '@/lib/match-summary';
...
export default function PausedSession({game,config}:{game:Simulation;config:MatchConfig}){
 ...
 const outcome=ffaCashoutPreview(config,stats);
 return <section className="paused-session" aria-label="Paused match summary">
  ...
  <div className="paused-session-body">
   {outcome&&<div className="paused-session-money"><span>SESSION {outcome.net<0?'LOSS':outcome.net>0?'PROFIT':'NET'}</span><strong className={outcome.net<0?'session-loss':'session-gain'}>{signedEuros(outcome.net)}</strong><small>{outcome.returned<0?'Wallet deduction':'Back to wallet'} <b>€{Math.abs(outcome.returned).toFixed(2)}</b></small></div>}
