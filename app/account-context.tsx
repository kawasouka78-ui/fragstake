...
export async function accountApi<T=AccountData>(body?:Record<string,unknown>,query=''):Promise<T>{const r=await fetch('/api/community'+query,{method:body?'POST':'GET',headers:body?{'Content-Type':'application/json'}:undefined,body:body?JSON.stringify(body):undefined,cache:'no-store'});const data=await r.json() as T & {error?:string};if(!r.ok)throw new Error(data.error||'Something went wrong. Please try again.');return data as T;}
...
export function AccountProvider({children}:{children:ReactNode}){const [data,setData]=useState<AccountData|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState('');const refresh=useCallback(async()=>{try{setData(await accountApi());setError('')}catch(e){setError((e as Error).message)}finally{setLoading(false)}},[]);useEffect(()=>{void refresh();const timer=setInterval(()=>{if(document.visibilityState==='visible')void refresh()},45000);return()=>clearInterval(timer)},[refresh]);return <Context.Provider value={{data,loading,error,refresh,update:setData}}>{children}</Context.Provider>}
