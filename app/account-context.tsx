...
const platform=()=>({party:{id:'party-preview',owner_id:mockPlayer.id,name:'Preview Squad',members:[{id:'pm-1',player_id:mockPlayer.id,name:mockPlayer.name,handle:mockPlayer.handle,status:'joined'},{id:'pm-2',player_id:'vex',name:'Vex',handle:'vex',status:'joined'},{id:'pm-3',player_id:'mira',name:'Mira',handle:'mira',status:'invited'}]},invites:[{id:'invite-1',name:'AceNova invited you to Night Queue'}],challenges:[{id:'challenge-1',sender_id:'vex',receiver_id:mockPlayer.id,sender_name:'Vex',receiver_name:mockPlayer.name,rules:JSON.stringify({team:'1v1',stake:10,target:10,bestOf:1,weaponRule:'rifle'}),status:'pending',expires_at:now+3600000}],inventory:catalog.slice(0,5).map((item,i)=>({sku:item.sku,equipped:i===0?1:0})),reports:[{id:'report-1',category:'bug',details:'Slide felt sticky near the Depot lower ramp.',status:'reviewing',created_at:now-2*day},{id:'report-2',category:'cheating',details:'Suspicious pre-fire in a 1v1 preview duel.',status:'open',created_at:now-5*day}],ratings:[{mode:'duel',matches:14,wins:9,kills:182,deaths:129,...demoRating(9,14)},{mode:'ffa',matches:21,wins:7,kills:311,deaths:246,...demoRating(7,21)}],catalog});
