...
export default function MatchReceipt({stats,mode,net=0,caption,returned,rate,lastDeathLoss,hideTotal=false}:Props){
 ...
 return <div className="match-receipt">
  {!hideTotal&&<div className={'receipt-total '+tone}><span>{mode==='practice'?'YOUR PERFORMANCE':net>0?'SESSION PROFIT':net<0?'SESSION LOSS':'SESSION NET'}</span><strong>{mode==='practice'?`${stats.kills} / ${stats.deaths}`:signedEuros(net)}</strong><p>{mode==='practice'?(caption??'Kills / deaths · Free practice'):caption??'Demo credits'}</p></div>}
