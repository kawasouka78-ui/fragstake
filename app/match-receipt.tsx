...
type Props={stats:CombatStats;mode:string;net?:number;caption?:string;returned?:number;rate?:number;lastDeathLoss?:number};
export default function MatchReceipt({stats,mode,net=0,caption,returned,rate,lastDeathLoss}:Props){
 const tone=net>0?'gain':net<0?'loss':'even';
 return <div className="match-receipt">
  <div className={'receipt-total '+tone}><span>{mode==='practice'?'YOUR PERFORMANCE':net>0?'SESSION PROFIT':net<0?'SESSION LOSS':'SESSION NET'}</span><strong>{mode==='practice'?`${stats.kills} / ${stats.deaths}`:signedEuros(net)}</strong><p>{mode==='practice'?(caption??'Kills / deaths · Free practice'):caption??'Demo credits'}</p></div>
