...
export default function DuelResult({result,config,pending,busy,error,retry,rematch,findNext,leave}:Props){
 ...
 return <div className={'duel-result '+outcome.tone}>
  <div className="duel-result-meta"><span>{getMap(result.mapId).name}</span><i/><span>{result.liveMode??config?.team??'1v1'} DUEL</span><span>{live?'PLAYER MATCH':'MATCH FUNDS'}</span></div>
  <div className="duel-result-hero">
   ...
   <DialogDescription>{incomplete?/cancelled/i.test(result.reason)?'You left before the round started. No result was recorded.':'You left before the final result.':/forfeit/i.test(result.reason)?'You left before the duel ended. Your stake was forfeited.':live?'Final score · Player duel':outcome.tone==='win'?'Duel secured. Your result is below.':outcome.tone==='loss'?'The next duel is a fresh start.':'The duel is closed. Your stake is safe.'}</DialogDescription>
  </div>
  <div className="duel-result-headline">
   <div><span>{live?'MATCH EARNINGS':outcome.label}</span><strong>{signedEuros(result.net??0)}</strong><small>{live?'Final match result':pending?'Settling match funds…':'Net change to your balance'}</small></div>
   <div className="duel-result-score"><span>{incomplete?'SCORE WHEN YOU LEFT':'FINAL SCORE'}</span><strong>{result.score}<em>:</em>{result.enemyScore}</strong><small>Your team · Opponents</small></div>
  </div>
  ...
  <p className="duel-result-hint">{pending?'Saving your result before the next match.':live?canRematch?'Rematch invites the same players. Everyone must join and ready up.':'Find another game to enter a new player match.':`Rematch keeps this map and rules · €${config?.stake??10} entry`}</p>
 </div>;
}
